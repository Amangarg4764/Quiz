...
//view all course
router.get('/viewcourse',passport.checkAuthentication,async function(req,res){
   ...
   /* let ep=await  Userdata.findById(req.user._id).populate('cenrolled').populate({
        path:'cenrolled',
        populate: { path:  'coursename',
		    model: 'course' }
    }).exec();
    console.log(ep.cenrolled);*/
